...
#[export_name = "turbo_program:command_handler/ggmvgxdev4/increment_faction_level"]
unsafe extern "C" fn on_increment_faction_level() -> usize {
    ...
    let mut current_faction_deserialized =
        states::FactionState::try_from_slice(&read_result.unwrap()).unwrap();
    ...
    if current_faction_deserialized.current_level == current_faction_deserialized.max_level {
        ...
        for player in current_meta_state_deserialized.player_list.iter() {
            ...
            let mut current_player_data_deserialized =
        ...
        // write change
        let write_result = os::server::fs::write_bytes(
            &format!("factions/{faction_in_question_as_str}"),
